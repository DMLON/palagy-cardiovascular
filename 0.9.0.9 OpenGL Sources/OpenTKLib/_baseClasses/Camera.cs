...
namespace OpenTKLib
{
    public class Camera
    {
        ...
        public void PerspectiveUpdate(int width, int height)
        {
            if (height != 0)
            {
                perspectiveAspectRatio = width / (float)height;
                if (ZNear == 0)
                    ZNear = 1;
                if(ZNear > ZFar)
                {
        ...
        /// <summary>
        /// Rotates the camera about the Y axis.  Assumes a fixed Y axis of Vector3d.Up.
        /// </summary>
        /// <param name="angle">The amount to rotate the camera by.</param>
        public void Yaw(float angle)
        {
            ...
            // of an unfixed yaw, simply replace Vector3d.Up with (orientation * Vector3d.UnitY)
            angle = angle * MoveSpeed;
