...
namespace OpenTKLib
{
    ...
    public struct QuaternionNew : IEquatable<QuaternionNew>
    {
        ...
        public Matrix4d Matrix4d
        {
            ...
            {
                return new Matrix4d(
                    new Vector4d(1.0f - 2.0f * (Y * Y + Z * Z), 2.0f * (X * Y - W * Z), 2.0f * (X * Z + W * Y), 0.0f),
                    new Vector4d(2.0f * (X * Y + W * Z), 1.0f - 2.0f * (X * X + Z * Z), 2.0f * (Y * Z - W * X), 0.0f),
                    new Vector4d(2.0f * (X * Z - W * Y), 2.0f * (Y * Z + W * X), 1.0f - 2.0f * (X * X + Y * Y), 0.0f),
        ...
        ///// </summary>
        ///// <returns>Three Vector3d structs in an array, representing this quaternion.</returns>
        //public Vector3d[] ToAxis()
        //{
        //    Matrix4d rotationMatrix = this.Matrix4d;
        //    return new Vector3d[] { new Vector3d(rotationMatrix[0].X, rotationMatrix[1].X, rotationMatrix[2].X),
        //        new Vector3d(rotationMatrix[0].Y, rotationMatrix[1].Y, rotationMatrix[2].Y),
        //        new Vector3d(rotationMatrix[0].Z, rotationMatrix[1].Z, rotationMatrix[2].Z) };
        //}
        ...
        public Vector4d ToAxisAngle()
        {
            ...
            if (den > 0.0001f)
            {
                result.Xyz = new Vector3d(q.X, q.Y, q.Z) / den;
            }
            else
            {
        ...
        /// <summary>
        /// Creates an orientation QuaternionNew from a target Matrix4d rotational matrix.
        /// </summary>
